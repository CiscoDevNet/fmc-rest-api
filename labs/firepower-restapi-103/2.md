...
As mentioned previously under UI section, you have to add FMC to the device using command line (CLI) on the device. Same steps of adding FMC to device apply for REST API device added to FMC as well.

Please watch this video on the device registration process [FMC device setup](https://youtu.be/7eTFSfL0EkU).
#### Sample of Python script for adding device through FMC, using REST calls
